...
namespace Drizzle3D {
	App::App(RenderingAPI rAPI, char* WindowName, int width, int height) {
		renderingAPI = rAPI;
		dispatch = std::make_shared<EventDispatcher>();
		LayerDispatcher = std::make_shared<LayerDispatch>();
		D3DWindow = std::make_shared<Window>(rAPI, dispatch, WindowName, width, height);
		imguilayer = std::make_shared<ImGuiLayer>(rAPI, D3DWindow);
		resourcemgr = std::make_shared<ResourceManager>();
		renderinglayer = std::make_shared<RenderingLayer>(rAPI, D3DWindow, resourcemgr);
		renderinglayer2d = std::make_shared<RenderingLayer2D>(rAPI, D3DWindow, resourcemgr);
